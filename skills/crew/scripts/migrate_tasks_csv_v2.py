...
Acciones:
  1. Backup del CSV original en .sfcrew/archive/tasks_v1_backup_YYYYMMDD.csv
  2. Agrega las 6 columnas nuevas
  3. Backfill hu_code por regex sobre `prompt` (HU-*, ADJ-*, DT-ACC-*, DT-CON-*, ÉPICA n)
  4. Backfill notion_page_id desde el mapa code→page_id (--map)
  5. Normaliza: agent='tbd' → vacío; elimina líneas vacías
...
def migrate(csv_path, map_path, dry_run=False):
    ...
    if header == V2_HEADER:
        ...
        return
    if header != V1_HEADER:
        sys.exit(f"Header inesperado, aborto sin tocar nada:\n{header}")
    ...
    for r in data:
        ...
        page_id, sync_state = "", ""
        if len(codes) >= 1 and codes[0] in notion_map:
            page_id = notion_map[codes[0]]
            sync_state = "ok"
            stats["notion"] += 1
        elif hu_code:
            sync_state = "dirty"  # tiene código pero sin tarjeta Notion enlazable

        out.append([row[c] for c in V1_HEADER] + [page_id, hu_code, "", "", "", sync_state])

    print(f"Filas: {len(data)} | hu_code: {stats['hu']} | notion_page_id: {stats['notion']} | tbd normalizados: {stats['tbd']}")
    if dry_run:
