# -*- coding: utf-8 -*-
"""
sync_csv.py — Helper determinista del motor de sync SFCrew 3.0 (skill notion-sync).

Subcomandos:
  stats <csv>                                  Resumen operativo
  plan <csv> --notion <snapshot.json>          Emite plan de sync (JSON a stdout)
       [--state <sync_state.json>]
  apply-csv <csv> --plan <plan.json>           Aplica cambios lado-CSV del plan
  commit-state --plan <plan.json>              Persiste hashes post-sync
       --state <sync_state.json>

Formato snapshot Notion:
...
CANONICAL = {
    ...
    "deployed": "deployed", "blocked": "blocked", "failed": "blocked",
    "partial": "partial", "qa": "qa", "tech-debt": "tech-debt",
    "returned": "returned", "de_vuelta": "returned",
}
STATE_TO_NOTION = {
    ...
    "returned": "En Progreso SFCrew",
}
APPROVAL_STATES = {"ready_to_merge"}
EXCEPTION_STATES = {"blocked", "partial"}

PROGRESS_ORDER = ["blocked", "pending", "assigned", "returned", "in_progress",
                  "partial", "tech-debt", "qa", "ready_to_merge", "deployed"]
        ...
        if expected_status and card.get("status") != expected_status:
            if notion_changed and csv_changed and not first_sync:
                conflicts.append({
                    ...
                    "politica": "CSV es dueño del estado de ejecución",
                })
                continue
